...
try:
    while :
        #comment out 1 of the two requests for either Data on Batumi or Tbilsi
        ...
        page = requests.get("https://www.myhome.ge/en/s/Apartment-for-sale-Kutaisi?Keyword=Kutaisi&AdTypeID=1&PrTypeID=1&Page="+str(page_nr)+"&mapC=42.2716455%2C42.705359&cities=8742174&GID=8742174") #apartments for sale in Kutaisi
        ...
        sizes.extend(sizes_current_site[:-3])
        page_nr += 1
        time.sleep(5)
except:
    print('done')
...
# sizes = [x.replace(".00","") for x in sizes] # doesn't work if there are dirty floats in the list
